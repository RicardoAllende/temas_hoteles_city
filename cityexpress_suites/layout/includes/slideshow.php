...
$themeurl = theme_cityexpress_suites_theme_url();
...
function slideshow_general_config() {
    ...
    $general = array();
    $general['slideshowStatus'] = theme_cityexpress_suites_get_setting('slideshowStatus');
    $autoslideshow = theme_cityexpress_suites_get_setting('autoslideshow');
    $slideinterval = theme_cityexpress_suites_get_setting('slideinterval');
...
function slideshow_body($general) {
    ...
    $data['maincontent'] = "";
    for ($s1 = 1; $s1 <= 3; $s1++) {
        $status = theme_cityexpress_suites_get_setting('slide' . $s1 . 'status');
        if ($status == "1") {
...
function slideshow_slide_settings($general = array(), $s1 = "") {
    if (!empty($s1) && !empty($general)) {
        $slide['slideStatus'] = theme_cityexpress_suites_get_setting('slide' . $s1 . 'status');
        $slide['slideurl1'] = theme_cityexpress_suites_get_setting('slide' . $s1 . 'url1');
        $slide['slideimg'] = theme_cityexpress_suites_render_slideimg($s1, 'slide' . $s1 . 'image');
        $slide['contentPosition'] = theme_cityexpress_suites_get_setting('slide'. $s1 .'contentPosition');
        $urltarget1 = theme_cityexpress_suites_get_setting('slide'. $s1 .'urltarget1');
        $contwidth = theme_cityexpress_suites_get_setting('slide'.$s1.'contFullwidth');

        $slidedesc = theme_cityexpress_suites_get_setting('slide' . $s1 . 'desc', 'format_html');
        $slideurltext1 = theme_cityexpress_suites_get_setting('slide' . $s1 . 'urltext1');
        $slidecaption = theme_cityexpress_suites_get_setting('slide' . $s1 . 'caption', true);
        $slide['slidecaption'] = theme_cityexpress_suites_lang($slidecaption);
        $slide['slideurltext1'] = theme_cityexpress_suites_lang($slideurltext1);
        $slide['slidedesc'] = theme_cityexpress_suites_lang($slidedesc);
        $s2 = (int) $s1 - 1;
        if ($contwidth == "auto") {
            $contwidth = "auto";
        } else {
...
?>
    ...
    (function($) {
        ...
        var imagePath = <?php echo "'$CFG->wwwroot/theme/cityexpress_suites/images/'"; ?>;
