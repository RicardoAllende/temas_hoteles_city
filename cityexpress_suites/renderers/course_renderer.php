...
/**
 * Theme cityexpress_suites course renderer class inherit from core course renderer class.
 * @copyright  2015 onwards LMSACE Dev Team (http://www.lmsace.com)
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class theme_cityexpress_suites_core_course_renderer extends core_course_renderer {

    public $coursesWerePrinted = false;
    ...
    //         $content .= $this->cityexpress_suites_coursecat_category($chelper, core_course_category::get($key), 1);
    //     }
     ...
     * @return string
     */
    public function coursecat_category(coursecat_helper $chelper, $coursecat, $depth) {
        ...
        if(!$this->coursesWerePrinted){
            ...
            foreach($courses as $course){
                ...
                // $content .= "<h5> Inscritos {$course->id}</h5>";
                // $content .= coursecat_coursebox()
            }
            $this->coursesWerePrinted;
        }
        $content .= "</div>";

        // $courses = $coursecat->get_courses();
        // $content .= "<h1>Imprimiendo categoría</h1>";
    ...
    public function top_course_menu() {
        ...
        '.get_string('viewall', 'theme_cityexpress_suites').'</a></li></ul>';
    ...
    public function frontpage_available_courses() {
       ...
       // require_once($CFG->libdir. '/coursecatlib.php');
        ...
        $header .= html_writer::tag('h2', get_string('courses'));
    ...
    public function frontpage_my_courses() {
        ...
        $content = html_writer::start_tag('div', array('class' => 'frontpage-enrolled-courses row') );
        $content .= html_writer::start_tag('div', array('class' => 'container'));
    ...
    //         $output .= html_writer::tag('h2', get_string('search_courses', 'theme_cityexpress_suites'));
    //     }
    ...
    //             'size' => $inputsize,
    //             'name' => 'search',
    //             'placeholder' => get_string('typesearch', 'theme_cityexpress_suites'),
    //             'value' => s($value)));
    ...
    protected function coursecat_coursebox(coursecat_helper $chelper, $course, $additionalclasses = '') {
        ...
        $content = '';
        $classes = ' coursebox clearfix col-sm-4 '. $additionalclasses;
        ...
        foreach ($course->get_course_overviewfiles() as $file) {
            ...
            if ($isimage) {
                $contentimages .= html_writer::tag('div',
                        html_writer::empty_tag('img', array('src' => $url, 'class'=> 'crop')),
                        array('class' => 'courseimage containerimg'));
            } else {
        ...
        // $course_percentage = theme_cityexpress_suites_get_user_progress($course);
...
// print enrolmenticons
        ...
        $completion_information = theme_cityexpress_suites_completion_get_activities_in_course($course->id);
        $activities = $completion_information->msg;
        $course_percentage = $completion_information->percentage;
        $content .= "
        <div class=\"row col-sm-12\">
            <div class=\"col-sm-6\"><h6>Progreso &nbsp; {$activities}</h6></div>
            <div class=\"col-sm-6\" style=\"text-align: end;\"><h6>{$course_percentage}%</h6></div>
        </div>
        <div class=\"progress container\">
            <div class=\"progress-bar\" role=\"progressbar\" style=\"width: {$course_percentage}%\" aria-valuenow=\"{$course_percentage}\"
...
} // Here the theme_cityexpress_suites_course renderer fucntion closed.
